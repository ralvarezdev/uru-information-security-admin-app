	...
	try:
		...
		for company in response.company_files:
			for filename in company.filenames:
				file_list.append(
					{"Company (CN)": company.common_name, "File Name": filename},
					)
		return file_list
...
with tab1:
	...
	if not active_files:
		...
		filenames = [f"{f['Company (CN)']}:{f['File Name']}" for f in
		             active_files]
		selected_file = st.selectbox(
			"Select a File to decrypt and decompress",
			options=filenames,
			)

		if st.button("Decrypt & Prepare Download"):
			if selected_file:
				common_name, filename = selected_file.split(":", 1)
			request = decrypter_pb2.DecryptFileRequest(
			...
			try:
				with st.spinner(f"Decrypting '{selected_file}'..."):
					...
					decrypted_content = b"".join(
						[r.file_content for r in response_iterator],
						)
				...
				try:
					...
					with zipfile.ZipFile(zip_buffer, 'r') as zip_ref:
						if not zip_ref.namelist():
							st.error(
								"Error: Received .zip is empty and cannot be decompressed.",
								)
							st.stop()
					...
					st.error(
						"Error: The received file is not a valid .zip. Cannot decompress.",
						)
					st.download_button(
		...
		with st.form("remove_one_form"):
			...
			if submitted_one:
				if cn_to_remove and fn_to_remove:
					request = decrypter_pb2.RemoveEncryptedFileRequest(
						common_name=cn_to_remove,
						filename=fn_to_remove,
						)
					try:
						stub.RemoveEncryptedFile(request)
						st.success(
							f"File '{fn_to_remove}' from company '{cn_to_remove}' deleted.",
							)
						st.cache_data.clear()
					except grpc.RpcError as e:
